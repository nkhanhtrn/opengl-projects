...
void createTexture(unsigned int* texture) {
    ...
    if (data) {
        ...
        //          type of texture   t_color type                       type of image source           
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D); // generate mimap after adding image to texture
    } else {
        std::cerr << "Failed to load texture image" << std::endl;
        exit(-1);
    }
    glBindTexture(GL_TEXTURE_2D, 0); // unbind after all done
    stbi_image_free(data); // free image from memory
}

void setVertex(unsigned int* VAO, unsigned int* VBO) {
   /* vertex buffers */
    float vertices[] = {
